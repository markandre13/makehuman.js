...
export class MorphRenderer extends RenderHandler {
    ...
    constructor(app: Application, model: MorphToolModel, faceRenderer: FaceRenderer) {
        super()
        this.app = app
        this.model = model
    ...
    paint(app: Application, view: RenderView): void {
        ...
        if (this.app.updateManager.updateFromLocalSettingsWithoutGL()) {
            this.needToUpdateMHMesh = true
            this.needToCalculateDistance = true
            this.needToCalculateMHBlendshapes = true
        }

        if (this.needToUpdateMHMesh) {
            console.log("DEBUG 0")
            this.pickMeshes[MH_MESH].vertices.update(app.humanMesh.vertexRigged)
            this.pickMeshes[MH_MESH].flat.update()
            this.needToUpdateMHMesh = false
        }
    ...
    for (const mhFaceIndex of mhFaceIndices) {
        ...
        for (let i = 0; i < fxyzAR.length;) {
            ...
            if (projection
                ...
                && isInTriangle(projection)) {
                if (match) {
                    if (Math.abs(projection!.d) < Math.abs(match.d)) {
                        arFaceIndex = _i
                        match = projection
                    }
                } else {
...
function computeBlendshapeMesh(baseMesh: WavefrontObj, mh2arProjection: Map<number, ARKitProjection>) {
    ...
    for (let i = offset, fo = 0; i < length + offset;) {
        ...
        if (!mhFaceIndicesSet.has(i0) || !mhFaceIndicesSet.has(i1) || !mhFaceIndicesSet.has(i2) || !mhFaceIndicesSet.has(i3)) {
            continue
        }
    ...
    for (let blendshape = 1; blendshape < Blendshape.SIZE - 1; ++blendshape) {
        ...
        const dxyz: number[] = []
        for (const mhFaceIndex of mhFaceIndices) {
            const mhOnAr = mh2arProjection.get(mhFaceIndex)
            if (mhOnAr === undefined) {
                continue
            }
            ...
            indices.push(mhFaceIndex)
            dxyz.push(neutralOnAr[0], neutralOnAr[1], neutralOnAr[2])
        }
        targets[blendshape] = new MorphTarget(indices, dxyz)
    }

    return new ComputedBlendshapeMesh(baseMesh.xyz, fxyz, targets)
}

/**
 * MH base mesh indices which overlap with the ARKit mesh
 */
...
export function computeBlendshapes(
    ...
    const leftEye = morphManager.getModifier("eyes/l-eye-trans-down|up")
    leftEye!.model!.value = -1
    const rightEye = morphManager.getModifier("eyes/r-eye-trans-down|up")
    rightEye!.model!.value = -1
    const forehead = morphManager.getModifier("forehead/forehead-trans-backward|forward")
    ...
    const fxyzAR = arkit._neutral!.fxyz
