...
class FaceMH implements BlendshapeMesh {
        ...
        if (this.needToCalculateMHBlendshapes && this.model.showAnimation.value) {
            ...
            for (let blendshape = 1; blendshape < Blendshape.SIZE - 1; ++blendshape) {
                ...
                for (const mhFaceIndex of mhFaceIndices) {
                    const ar = this.mh2arProjection.get(mhFaceIndex)
                    if (ar === undefined) {
                        continue
                    }
                    const O0 = getP(arkit.fxyz, xyz0, ar)
                    const O1 = getP(arkit.fxyz, xyz1, ar)

                    vec3.sub(O0, O1, O0)

                    indices.push(mhFaceIndex)
                    dxyz.push(O0[0], O0[1], O0[2])
                }
                facemh._targets[blendshape] = new MorphTarget(indices, dxyz)
            }
            console.log(`MorphRenderer::paint(): this.faceRenderer.setBlendshapeMesh(facemh)`)
            this.faceRenderer.setBlendshapeMesh(facemh)
    ...
    calculateDistance(gl: WebGL2RenderingContext) {
        ...
        for (const mhFaceIndex of mhFaceIndices) {
            ...
            if (match) {
                ...
                this.mh2arProjection.set(mhFaceIndex, { idx: arFaceIndex!, a: match.a, b: match.b })
            }
        }
