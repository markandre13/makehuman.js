/*
 *  XML Parser
 *  Copyright (C) 2020, 2025 Mark-André Hopf <mhopf@mark13.org>
 *
...
export class Node {
    ...
    toString(flat: boolean = false): string {
        throw Error(
            `toString() is not implemented (${this.filename}:${this.row}:${this.col})`
        )
    }
    innerText(): string {
        throw Error(
            `innerText() is not implemented (${this.filename}:${this.row}:${this.col})`
        )
    }
    expectEqual(node: Node) {
        throw Error(`${this.constructor.name}.expectEqual(${node.constructor.name}) not implemented yet`)
    }
}
...
export class Text extends Node {
    ...
    override toString(flat: boolean = false): string {
        return this.text
    }
    ...
    override printTree(indent: number) {
        let indentStr = ''
        for (let i = 0; i < indent; ++i) indentStr = `${indentStr}    `
        console.log(`${indentStr}text: ${this.text}`)
    }
    override expectEqual(node: Node) {
        if (!(node instanceof Text)) {
            throw Error(`expected Text`)
        }
        if (this.text.trim() !== node.text.trim()) {
            throw Error(`expected text nodes to equal: size ${this.text.length} != ${node.text.length}, '${this.text}' != '${node.text}'`)
        }
    }
}
...
export class Comment extends Node {
    ...
    override expectEqual(node: Node) {
        if (!(node instanceof Comment)) {
            throw Error(`expected Comment`)
        }
        if (this.text.trim() !== node.text) {
            throw Error(`expected text nodes to equal: '${this.text}' != '${node.text}'`)
        }
    }
}
...
export class Tag extends Node {
    ...
    getAttribute(key: string): string {
        ...
        if (value === undefined) throw Error(`missing attribute '${key}'`)
        return value
    }
    ...
    getChild(name: string): Tag {
        ...
        if (match === undefined) throw Error(`missing tag '${name}'`)
        return match
    }
    ...
    override innerText(): string {
        let text = ''
        for (let child of this.children) text += child.toString()
        return text
    }
    ...
    override printTree(indent: number) {
        ...
        for (let child of this.children) child.printTree(indent + 1)
    }
    ...
    override expectEqual(node: Node) {
        ...
        if (this.name !== node.name) {
            throw Error(`<${this.name}> != <${node.name}>`)
        }

        if (this.attributes.size !== node.attributes.size) {
            throw Error(`<${this.name}>: number of attributes does not match`)
        }
        for(const [key, lv] of this.attributes.entries()) {
            const rv = node.attributes.get(key)
            if (lv !== rv) {
                throw Error(`<${this.name}>: values for attibute ${key} differ: '${lv}' != '${rv}'`)
            }
        }

        let li=0, ri=0
        const lhs = this, rhs = node
        // TODO: copy pasta
        while(true) {
            // skip whitespace
            while(lhs.children[li] instanceof Text && (lhs.children[li] as Text).text.trim().length == 0) {
                ++li;
            }
            while(rhs.children[li] instanceof Text && (rhs.children[li] as Text).text.trim().length == 0) {
                ++ri;
            }
            ...
            if (li >= lhs.children.length) {
                throw Error(`different number of children l`)
            }
            if (ri >= rhs.children.length) {
                throw Error(`different number of children: missing left ${lhs.children[li].toString(true)}`)
            }
            ...
            try {
                ...
                if (e instanceof Error) {
                    throw Error(`${this.toString(true)} > ${e.message}`)
                } else {
                    throw e
                }
            }
...
class XMLLexer {
    ...
    ungetc() {
        ...
        if (this.data.charCodeAt(this.pos) == 0x0a) {
            ...
            for (
                let i = this.pos;
                i >= 0 && this.data.charCodeAt(this.pos) != 0x0a;
                --i
            ) {
                ++this.col
            }
    ...
    lex(): Node | undefined {
        ...
        while (true) {
            ...
            switch (state) {
                    ...
                    switch (char) {
                        ...
                        default:
                            if (!this.isNameStartChar(code))
                                this.error(
                                    `Expected tag name, comment or prolog after '<'`
                                )
                ...
                case 5: //<T /?
                    if (char != '>') this.error(`Expected '>' at end of tag`)
                    return tag

                case 6: //<T ?
                    if (this.isWhiteSpace(code)) break
                    if (this.isNameStartChar(code)) {
                        attrName += char
                        state = 7
                    } else if (char === '/') {
                ...
                case 14: // <!-
                    if (char !== '-') this.error("Expected '-' after '<!-'")
                    state = 15
                    comment = new Comment(this.filename, this.row, this.col)
                    break
                case 15: // <!--?
                    if (char == '-') state = 16
                    else comment!.text += char
                    break
                case 16: // <!--...-?>
                    if (char == '-') {
                        state = 17
                ...
                case 17:
                    if (char != '>')
                        this.error(
                            "'--' is only allowed in comment to close them"
                        )
                    return comment

                case 30: // <![?
                    cdata = new CData(this.filename, this.row, this.col)
                    if (char != 'C') this.error("Expected 'C' after <![")
                    state = 31
                    break
                case 31: // <![C?
                    if (char != 'D') this.error("Expected 'D' after <![C")
                    state = 32
                    break
                case 32: // <![CD?
                    if (char != 'A') this.error("Expected 'A' after <![CD")
                    state = 33
                    break
                case 33: // <![CDA?
                    if (char != 'T') this.error("Expected 'T' after <![CDA")
                    state = 34
                    break
                case 34: // <![CDAT?
                    if (char != 'A') this.error("Expected 'A' after <![DAT")
                    state = 35
                    break
                case 35: // <![CDATA?
                    if (char != '[') this.error("Expected '[' after <![CDATA")
                    state = 36
                    break
                case 36: // <![CDATA[...?
                    if (char === ']') state = 37
                    else cdata!.text += char
                    break
                case 37: // <![CDATA[...]?
                    if (char === ']') {
                        state = 38
            ...
            switch (token.type) {
                ...
                case TagType.End:
                    ...
                    if (last === undefined || tagStack.length < 1)
                        throw Error(
                            `unexpected closing tag ${token.name} (${token.filename}:${token.row}:${token.col})`
                        )
                    if (last.name !== token.name)
                        throw Error(
                            `wrong closing tag: expected ${token.name} but got ${last.name} (${token.filename}:${token.row}:${token.col})`
                        )
